...
db.setupDB = function(result)
{
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `groups` (`group_id` INT(10) AUTO_INCREMENT, `creator_id` INT(10), `group_name` VARCHAR(20), PRIMARY KEY(`group_id`),FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`) );");
    connection.query("CREATE TABLE IF NOT EXISTS `comments`(`comment_id` INT(10) AUTO_INCREMENT, `creator_id` INT(10), `post_id` INT(10), `comment_text` VARCHAR(280), `date_created` DATE,PRIMARY KEY(`comment_id`),FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`), FOREIGN KEY(`post_id`) REFERENCES `posts`(`post_id`) );");
    connection.query("CREATE TABLE IF NOT EXISTS `events`(`event_id` INT(10) AUTO_INCREMENT, `group_id` INT(10), `date_created` DATE, `event_date` DATE, `event_desc` VARCHAR(200),PRIMARY KEY(`event_id`), FOREIGN KEY(`group_id`) REFERENCES `groups`(`group_id`) );");
