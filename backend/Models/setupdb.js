...
db.setupDB = function(result)
{
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `tags_users_bridge`( `users_id` INT(10),`tag_id` INT(10), `inactive` TINYINT(1), PRIMARY KEY(`users_id`, `tag_id`),FOREIGN KEY(`users_id`) REFERENCES `users`(`id`),FOREIGN KEY(`tag_id`) REFERENCES `tags`(`tag_id`) );");
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `politicians`(`user_id` INT(10), `politician_type` VARCHAR(50), `office_phone` VARCHAR(20), `office_email` VARCHAR(50), `inactive` TINYINT(1), PRIMARY KEY(`user_id`), FOREIGN KEY(`user_id`) REFERENCES `users`(`id`));");
    connection.query("CREATE TABLE IF NOT EXISTS `groups_members_bridge`(`group_id` INT(10), `member_id` INT(10), `inactive` TINYINT(1), PRIMARY KEY(`group_id`, `member_id`), FOREIGN KEY(`group_id`) REFERENCES `groups`(`group_id`), FOREIGN KEY(`member_id`) REFERENCES `users`(`id`));"); 
