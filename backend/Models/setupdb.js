...
db.setupDB = function(result)
{
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `posts` (`post_id` INT(10) AUTO_INCREMENT, `creator_id` INT(10),`date_created` DATETIME, `title` VARCHAR(100), `post_text` VARCHAR(300), `inactive` TINYINT, PRIMARY KEY(`post_id`),FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`) );");
    connection.query("CREATE TABLE IF NOT EXISTS `groups` (`group_id` INT(10) AUTO_INCREMENT, `creator_id` INT(10), `group_name` VARCHAR(20), `inactive` TINYINT, PRIMARY KEY(`group_id`),FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`) );");
    connection.query("CREATE TABLE IF NOT EXISTS `comments`(`comment_id` INT(10) AUTO_INCREMENT, `creator_id` INT(10), `post_id` INT(10), `comment_text` VARCHAR(300), `date_created` DATETIME, `inactive` TINYINT, PRIMARY KEY(`comment_id`),FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`), FOREIGN KEY(`post_id`) REFERENCES `posts`(`post_id`) );");
    connection.query("CREATE TABLE IF NOT EXISTS `events`(`event_id` INT(10) AUTO_INCREMENT, `group_id` INT(10), `creator_id` INT(10), `date_created` DATETIME, `event_date` DATETIME, `event_desc` VARCHAR(200), `inactive` TINYINT, PRIMARY KEY(`event_id`), FOREIGN KEY(`group_id`) REFERENCES `groups`(`group_id`), FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`) );");
    connection.query("CREATE TABLE IF NOT EXISTS `polls` (`poll_id` INT(10) AUTO_INCREMENT,  `creator_id` INT(10), `question` VARCHAR(150), `date_created` DATETIME, `inactive` TINYINT, PRIMARY KEY(`poll_id`), FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`) );");
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `groups_admins`(`group_id` INT(10), `admin_id` INT(10), `current` TINYINT, `date_added` DATE, PRIMARY KEY(`group_id`, `admin_id`), FOREIGN KEY(`group_id`) REFERNCES `groups`(`group_id`), FOREIGN KEY(`admin_id`) REFERENCES `users`(`id`));")
    connection.query("CREATE TABLE IF NOT EXISTS `announcements`(`announcement_id` INT(10) AUTO_INCREMEMNT, `creator_id` INT(10), `grroup_id` INT(10), `announcement_text` VARCHAR(300), `date_created` DATETIME, `inactive` TINYINT, PRIMARY KEY(`announcement_id`), FOREIGN KEY(`creator_id`) REFERENCES `users`(`id`), FOREIGN KEY(`group_id`) REFERENCES `groups`(`group_id`) );");
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `groups_memebers_bridge`(`group_id` INT(10), `member_id` INT(10), `inactive` TINYINT, PRIMARY KEY(`group_id`, `member_id`), FOREIGN KEY(`group_id`) REFERENCES `groups`(`group_id`), FOREIGN KEY(`memeber_id`) REFERENCES `members`(`memeber_id`));"); 
