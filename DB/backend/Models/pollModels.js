...
var Poll = function(poll){
    ...
    this.count_answer2 = poll.count_answer2
     ...
     //we just have 2 answer which is a T/F question
};


Poll.createPoll = function(newPoll, result) {
  sql.query("INSERT INTO `ballotBuddy`.`polls` (`poll_id`,`creater_id`,`question`,`date_created`,`answer1`,`answer2`,`count_answer1`,`count_answer2`) VALUES ('" + newPoll.poll_id + ", " + newPoll.creator_id + ", " + newPoll.question + ", " + newPoll.date_created + ", " + newPoll.answer1 + ", " + newPoll.answer2 + ", " + newPoll.count_answer1 + ", " + newPoll.count_answer2 + ");",
    function(err, res) {
      if (err){
        result(err, null);
      }else{
...
Poll.getAllPolls = function getAllPolls(result) {
  ...
  User.getPollByID = function(poll_id, result) {
